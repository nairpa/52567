...
class CustomLanguageVisitor extends LanguageVisitor {
    constructor() {
        super();
    ...
    visitMulDiv(ctx) {
        ...
        return ctx.op.type === CalculatorParser.MUL ? left * right : left / right;
    }

    visitAddSub(ctx) {
        const left = this.visit(ctx.expr(0));
        const right = this.visit(ctx.expr(1));
        return ctx.op.type === CalculatorParser.ADD ? left + right : left - right;
    }
}
